...
class TicketMachine(LoggerMixin):
    ...
    def import_plugins(self):
        import ticketmachine.machines

        # pylint: disable=broad-except
        for module in ticketmachine.machines.__all__:
            try:
                module_id = "{0}.{1}".format('ticketmachine.machines', module)
                importlib.import_module(module_id)
                self.debug(module_id + " loaded successfully.")
            except Exception as exc:
                self.warning(
                    "The ticketmachine.{0} module could not be loaded: {1} "
